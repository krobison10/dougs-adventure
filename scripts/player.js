...
class Player extends Character {
    ...
    update() {
        ...
        let collision = this.checkCollide();
        if(!collision) {
    ...
    checkCollide() {
        let futurePos = {};
        futurePos.x = this.pos.x + this._velocity.x * gameEngine.clockTick;
        futurePos.y = this.pos.y + this._velocity.y * gameEngine.clockTick;
        let box = new BoundingBox(new Vec2(futurePos.x, futurePos.y), this.size);
